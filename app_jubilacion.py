...
#  TODAS as chaves internas SEM acentos
# ─────────────────────────────────────────────────────────────────────────────
rows = []
for _, row in df_ativos.iterrows():
    ...
    # Classe, País, Setor — usando col_* (nomes normalizados)
    def _get_str(col):
        if col and col in df_ativos.columns and pd.notna(row[col]):
            v = str(row[col]).strip()
            return v if v not in ("", "nan", "None") else "N/A"
        return "N/A"

    classe = _get_str(col_classe)
    pais   = _get_str(col_pais)
    setor  = _get_str(col_setor)
    ...
    valor_atual_usd = None
    if preco_usd_atual is not None and qtd is not None:
        valor_atual_usd = preco_usd_atual * qtd

    custo_total_usd = None
    if preco_medio_usd is not None and qtd is not None:
        custo_total_usd = preco_medio_usd * qtd
    ...
    div_pos_anual = None
    if div_anual_usd is not None and qtd is not None:
        div_pos_anual = div_anual_usd * qtd

    # ── TODAS AS CHAVES SEM ACENTOS ─────────────────────────────────────────
    rows.append({
        "ticker":            ticker,
        "nome":              nome,
        "classe":            classe,
...
# Mapa de nomes internos → rótulos para exibição
COL_LABEL = {
    "ticker":            "Ticker",
    "nome":              "Nome",
    "classe":            "Classe",
...
with tab1:
    ...
    # Cria cópia renomeada APENAS para exibição
    df_display = df.rename(columns=COL_LABEL).copy()

    # Formata colunas USD — acessa df original (sem acentos) → escreve em df_display
    for col_orig, col_label in [
        ("preco_atual_usd",   COL_LABEL["preco_atual_usd"]),
        ("preco_medio_usd",   COL_LABEL["preco_medio_usd"]),
        ("valor_atual_usd",   COL_LABEL["valor_atual_usd"]),
        ("custo_total_usd",   COL_LABEL["custo_total_usd"]),
        ("lp_usd",            COL_LABEL["lp_usd"]),
        ("div_anual_pos_usd", COL_LABEL["div_anual_pos_usd"]),
    ]:
        df_display[col_label] = df[col_orig].apply(formatar_usd)

    # Formata colunas %
    for col_orig, col_label in [
        ("var_dia_pct",   COL_LABEL["var_dia_pct"]),
        ("lp_pct",        COL_LABEL["lp_pct"]),
        ("div_yield_pct", COL_LABEL["div_yield_pct"]),
    ]:
        df_display[col_label] = df[col_orig].apply(formatar_pct)

    df_display[COL_LABEL["qtd"]] = df["qtd"].apply(
        lambda x: f"{x:,.0f}" if x is not None and not np.isnan(float(x)) else "—"
    )
