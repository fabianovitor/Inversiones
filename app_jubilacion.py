...
def parse_sheet(content):
    ...
    try:
        ...
        # Guarda nomes originais antes de normalizar
...
def encontrar_coluna(df_cols_norm, candidatos):
    """
    Encontra coluna normalizada no DataFrame (colunas já normalizadas).
...
#  Planilha: Ticker | Qtd | Preço Médio | Moeda | Classe | País | Setor
# ─────────────────────────────────────────────────────────────────────────────
colunas_norm = list(df_raw.columns)  # já normalizadas

CANDIDATOS = {
    "ticker":      ["Ticker", "ticker", "Ativo", "Symbol", "Símbolo", "simbolo", "Code", "Código"],
    ...
    "pais":        ["País", "Pais", "pais", "Country", "country", "Mercado", "Região"],
