...
# Layout padrão para todos os gráficos (eixos bem visíveis)
LAYOUT_PADRAO = dict(
    plot_bgcolor="#f8fafc",
    paper_bgcolor="white",
...
def render_progress_bar(label, valor_atual, meta, cor, nome_meta="Meta"):
    ...
    st.markdown(f"""
<div style="background:#e2e8f0; border-radius:8px; height:32px; position:relative; margin-bottom:4px;">
  <div style="background:{cor}; border-radius:8px; height:32px; width:{bar_width:.1f}%;
              min-width:0; transition:width 0.3s ease;">
  </div>
  <div style="position:absolute; top:0; left:0; right:0; height:32px;
              display:flex; align-items:center; padding:0 12px;">
    <span style="color:#1e293b; font-weight:700; font-size:0.9rem;
...
with st.expander("🔍 Debug — Clique apenas se houver problema", expanded=False):
    ...
    if col_ticker_debug in df_raw.columns:
        st.write(f"**Tickers encontrados (bruto):** {df_raw[col_ticker_debug].tolist()}")
...
with g2:
    if col_cat in df.columns and "Patrimônio (USD)" in df.columns:
        ...
        if not df_cat.empty:
            ...
            fig_setor.update_traces(textfont=dict(color="#1e293b"))
...
with g3:
    if "Renda Mensal Est. (USD)" in df.columns and col_ticker in df.columns:
        ...
        df_renda["Renda Mensal Est. (USD)"] = pd.to_numeric(
            df_renda["Renda Mensal Est. (USD)"], errors="coerce"
        )
        ...
        if not df_renda.empty:
            ...
            fig_renda.update_layout(
                title=dict(text="Renda Mensal Estimada por Ativo (USD)", font=dict(color="#1e293b", size=14)),
                xaxis_title="USD / mês",
                **LAYOUT_PADRAO,
                margin=dict(r=100)
            )
...
with g4:
    if "Retorno (%)" in df.columns and col_ticker in df.columns:
        ...
        if not df_ret.empty:
            ...
            text_vals = df_ret["Retorno (%)"].apply(
            ...
            fig_ret.update_layout(
                title=dict(text="Retorno por Ativo (%)", font=dict(color="#1e293b", size=14)),
                **LAYOUT_PADRAO,
                yaxis=dict(
            ...
            fig_ret.update_yaxes(range=[min_val - padding, max_val + padding])
...
fig_cenarios.update_layout(
    title=dict(text="🎯 Projeção de Renda Mensal — 3 Cenários (USD)", font=dict(color="#1e293b", size=14)),
    xaxis_title="Anos",
...
if "Renda Mensal Est. (USD)" in df.columns and col_ticker in df.columns:
    # Cria df_contrib com valores NUMÉRICOS (sem formatar ainda)
    cols_base = [col_ticker, "Renda Mensal Est. (USD)", "Patrimônio (USD)"]
    if "DY (%)" in df.columns:
        cols_base.append("DY (%)")
    df_contrib = df[cols_base].copy()

    # Garante que a coluna de renda seja numérica
    df_contrib["Renda Mensal Est. (USD)"] = pd.to_numeric(
        df_contrib["Renda Mensal Est. (USD)"], errors="coerce"
    )

    # Calcula % da Meta Final como número (0-100)
    ...
    # Remove linhas com NaN na coluna chave
...
# Começa com df original (valores numéricos)
df_tabela = df.copy()

# Junta % da Meta Final (numérico, antes de formatar)
if df_contrib is not None and col_ticker in df_tabela.columns:
    ...
    # df_contrib ainda tem valores numéricos aqui
...
# Formata colunas originais da planilha (numéricas)
col_pm  = COLUMN_MAP["preco_medio"]
col_pa  = COLUMN_MAP["preco_atual"]
col_vt  = COLUMN_MAP["valor_total"]
col_div = COLUMN_MAP["dividendos"]
...
for col in [col_pm, col_pa, col_vt, col_div]:
    if col in df_display.columns:
        df_display[col] = df_display[col].apply(
            lambda x: f"$ {float(x):,.2f}" if is_valid_number(x) else ""
        )

if col_yoc in df_display.columns:
