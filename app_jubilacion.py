...
if not df_principal.empty:
    ...
    df_show["Preço (USD)"] = df_show["Preço (USD)"].apply(fmt_usd)
    ...
    df_show["Renda Mês"] = df_show["Renda Mês"].apply(fmt_usd)
    ...
    if not df_pie.empty:
        fig = px.pie(df_pie, values="valor_atual", names="ticker",
                     title="Distribuição da Carteira Principal", hole=0.4)
...
if not df_eric.empty:
    ...
    df_e.columns = ["Ticker", "Nome", "Moeda", "Qtd", "PM (USD)", "Preço (USD)",
                    "Valor (USD)", "Lucro (USD)", "Lucro %", "Renda Mês"]
    df_e["PM (USD)"] = df_e["PM (USD)"].apply(fmt_usd)
    df_e["Preço (USD)"] = df_e["Preço (USD)"].apply(fmt_usd)
    ...
    df_e["Renda Mês"] = df_e["Renda Mês"].apply(fmt_usd)
    st.dataframe(df_e, use_container_width=True, hide_index=True)

# PROJEÇÃO
st.markdown("---")
st.subheader("🔮 Projeção Patrimonial")
anos_proj = st.slider("Anos de projeção", 5, 40, 20)
...
if not df_proj.empty:
    fig2 = px.line(df_proj, x="Ano", y="Patrimônio (USD)",
    ...
    st.dataframe(df_proj.style.format({"Patrimônio (USD)": "${:,.2f}",
