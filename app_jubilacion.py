...
def to_float(val):
    ...
    try:
        ...
        s = s.replace("€", "").replace("£", "").replace("kr", "").replace("R$", "")
        s = s.replace("\xa0", "").replace("\u202f", "")
        if s in ("", "-", "nan", "NaN", "None", "N/A", "#N/A", "#VALUE!", "#REF!"):
            return None
...
def is_valid_ticker(val):
    if val is None:
        return False
    s = str(val).strip()
    if s in ("", "nan", "NaN", "None", "N/A", "#N/A", "#VALUE!", "#REF!", "Ticker"):
        return False
    # Ignora linhas que parecem ser cabeçalhos repetidos ou separadores
    if s.lower() in ("ticker", "total", "totals", "subtotal"):
        return False
    if len(s) > 15:
        return False
...
# ─────────────────────────────── DEBUG ──────────────────────────────────────
with st.expander("🔍 Debug — Clique para ver detalhes do carregamento"):
    st.write(f"**Spreadsheet ID:** `{SPREADSHEET_ID}`")
    st.write(f"**GID:** `{GID_PADRAO}`")
    st.write(f"**URL gerada:** `{url_usada}`")
    if erro:
        st.error(f"**Erro:** `{erro}`")
    else:
        st.success("**Status:** Dados carregados com sucesso!")
        st.write(f"**Tamanho do conteúdo:** {len(csv_text)} caracteres")
        st.write("**Primeiros 500 caracteres:**")
        st.code(csv_text[:500] if csv_text else "(vazio)")
...
**GID atual:** `{GID_PADRAO}`
    ...
    st.write("Cole esta URL no seu navegador para testar:")
    st.code(url_usada)
    st.stop()
...
try:
    ...
    with st.expander("📄 Conteúdo recebido"):
        st.code(csv_text[:1000])
    st.stop()
...
with st.expander("🔍 Debug — Dados carregados (RAW antes do processamento)"):
    st.write(f"**Colunas encontradas:** {list(df_raw.columns)}")
    st.write(f"**Total de linhas (bruto):** {len(df_raw)}")
    st.write("**Todas as linhas brutas:**")
    st.dataframe(df_raw)

df = process_dataframe(df_raw)

with st.expander("🔍 Debug — Dados após processamento"):
    st.write(f"**Total de linhas processadas:** {len(df)}")
    st.write(f"**Colunas após processamento:** {list(df.columns)}")
    st.dataframe(df)
