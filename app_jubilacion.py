...
def safe_isna(val):
    ...
    try:
        result = pd.isna(val)
        # pd.isna pode retornar array se val for array-like
...
def to_float(val):
    ...
    if val is None:
        return None
    # Verifica NaN antes de qualquer coisa
    if safe_isna(val):
        return None
    # Se já é número
    if isinstance(val, (int, float)):
        try:
            f = float(val)
            # Verifica se é NaN após conversão
            if f != f:  # NaN != NaN é True
                return None
            return f
        except Exception:
            return None
...
def is_valid_number(val):
    """Retorna True se val é um número válido (não None, não NaN)."""
    if val is None:
        return False
    if safe_isna(val):
        return False
...
def find_column(df, candidates):
    ...
    for candidate in candidates:
        # Pula candidatos None ou inválidos
...
def process_dataframe(df: pd.DataFrame):
    ...
    def get_num_series(col):
        """Converte coluna para série numérica de forma segura."""
    ...
    qtd_num = get_num_series(col_qtd)
    pm_num  = get_num_series(col_pm)
    div_num = get_num_series(col_div)
    yoc_num = get_num_series(col_yoc)
    pa_num  = get_num_series(col_pa)
    vt_num  = get_num_series(col_vt)

    # Patrimônio
    pat_list = []
    if col_vt and col_vt in df.columns:
        for v in vt_num:
            pat_list.append(v if is_valid_number(v) else None)
    elif col_qtd and col_pa and col_qtd in df.columns and col_pa in df.columns:
        for q, p in zip(qtd_num, pa_num):
            if is_valid_number(q) and is_valid_number(p):
                pat_list.append(float(q) * float(p))
            else:
                pat_list.append(None)
    else:
        pat_list = [None] * len(df)
    df["Patrimônio (USD)"] = pat_list
    ...
    if col_qtd and col_pm and col_qtd in df.columns and col_pm in df.columns:
        for q, p in zip(qtd_num, pm_num):
            if is_valid_number(q) and is_valid_number(p):
                custo_list.append(float(q) * float(p))
            else:
                custo_list.append(None)
    else:
        custo_list = [None] * len(df)
    df["Custo Total (USD)"] = custo_list
    ...
    for pat, cus in zip(df["Patrimônio (USD)"], df["Custo Total (USD)"]):
        if is_valid_number(pat) and is_valid_number(cus):
            pat_f = float(pat)
            cus_f = float(cus)
            lucros.append(pat_f - cus_f)
            retornos.append(round((pat_f - cus_f) / cus_f * 100, 2) if cus_f != 0 else None)
        else:
            lucros.append(None)
    ...
    # ============================================================
    # RENDA MENSAL — lógica corrigida e robusta
    # ============================================================
    rendas = []
    ...
    if col_div and col_div in df.columns:
        for i in range(len(df)):
            try:
                ...
                if is_valid_number(div):
                    rendas.append(round(float(div) / 12, 4))
                else:
                    # Fallback: tenta calcular pelo YoC
                    yoc_val = None
                    if col_yoc and col_yoc in df.columns:
                        try:
                            yoc_val = yoc_num.iloc[i]
                        except Exception:
                            yoc_val = None
                    pat_val = df["Patrimônio (USD)"].iloc[i]
                    if is_valid_number(yoc_val) and is_valid_number(pat_val) and float(pat_val) > 0:
                        rendas.append(round(float(pat_val) * float(yoc_val) / 100 / 12, 4))
                    else:
                        rendas.append(0.0)
            except Exception:
                rendas.append(0.0)
    ...
    elif col_yoc and col_yoc in df.columns:
        for i in range(len(df)):
            try:
                pat = df["Patrimônio (USD)"].iloc[i]
                yoc = yoc_num.iloc[i]
                if is_valid_number(pat) and is_valid_number(yoc) and float(pat) > 0:
                    rendas.append(round(float(pat) * float(yoc) / 100 / 12, 4))
                else:
                    rendas.append(0.0)
            except Exception:
                rendas.append(0.0)
        ...
        df["Renda Mensal Est. (USD)"] = [0.0] * len(df)
    ...
    mask_pat = df["Patrimônio (USD)"].apply(lambda x: is_valid_number(x) and float(x) > 0)
    df = df[mask_pat].reset_index(drop=True)
...
def safe_sum(series):
    try:
        vals = [float(v) for v in series if is_valid_number(v)]
        return sum(vals)
    except Exception:
        return 0.0
...
# ── DEBUG ─────────────────────────────────────────────────────────────────────
with st.expander("🔍 Debug — Dados brutos (clique para ver)", expanded=False):
    st.write(f"**Colunas ({len(df_raw.columns)}):** {list(df_raw.columns)}")
    st.write("**Primeiras 5 linhas:**")
    st.dataframe(df_raw.head(5))

    col_div_debug = find_column(df_raw, ["Dividendos TTM", "Dividendos", "Dividends TTM", "Dividends"])
    if col_div_debug:
        st.write(f"**Coluna Dividendos:** `{col_div_debug}`")
        st.write("**Valores brutos (primeiros 10):**")
        for i, v in enumerate(df_raw[col_div_debug].head(10)):
            converted = to_float(v)
            st.write(f"  [{i}] `{repr(v)}` → `{converted}`")
    else:
        st.warning("Coluna de Dividendos NÃO encontrada!")

# Processa o dataframe com tratamento de erro global
try:
...
with st.expander("🔍 Debug — Resultado do processamento (clique para ver)", expanded=False):
    ...
    st.write(f"**Total de ativos após filtro:** {len(df)}")

    if "Renda Mensal Est. (USD)" in df.columns:
        st.success("✅ Coluna 'Renda Mensal Est. (USD)' calculada!")
        total_renda = safe_sum(df["Renda Mensal Est. (USD)"])
        st.write(f"**Total Renda Mensal:** $ {total_renda:,.2f}")
        if col_ticker and col_ticker in df.columns:
            cols_show = [c for c in [col_ticker, "Renda Mensal Est. (USD)", "Patrimônio (USD)"] if c in df.columns]
            st.dataframe(df[cols_show].head(15))
    else:
...
# ── KPIs ──────────────────────────────────────────────────────────────────────
patrimonio_total = safe_sum(df["Patrimônio (USD)"])
custo_total      = safe_sum(df["Custo Total (USD)"])
lucro_total      = patrimonio_total - custo_total
retorno_pct      = (lucro_total / custo_total * 100) if custo_total > 0 else 0.0
renda_mensal     = safe_sum(df["Renda Mensal Est. (USD)"])
...
with g1:
    if col_ticker and "Patrimônio (USD)" in df.columns:
        ...
        df_plot = df_plot[df_plot["Patrimônio (USD)"] > 0]
...
with g2:
    if col_cat and "Patrimônio (USD)" in df.columns:
        ...
        df_cat = df_cat[df_cat["Patrimônio (USD)"] > 0]
        if not df_cat.empty:
            ...
            st.info("Sem dados de categoria para o gráfico.")
    else:
        st.info("Coluna de categoria não mapeada.")
...
with g3:
    ...
    if col_renda in df.columns and col_ticker:
        ...
        df_renda[col_renda] = pd.to_numeric(df_renda[col_renda], errors="coerce").fillna(0)
        df_renda = df_renda[df_renda[col_renda] > 0].sort_values(col_renda, ascending=True)
        if not df_renda.empty:
            fig_renda = go.Figure(go.Bar(
                x=df_renda[col_renda], y=df_renda[col_ticker], orientation="h",
                marker_color="#22c55e",
                text=df_renda[col_renda].apply(lambda x: f"$ {x:.2f}"),
            ...
            st.info("Nenhum ativo com renda mensal > 0.")
    else:
        st.warning(f"Renda mensal não calculada.")
...
if col_renda in df.columns and col_ticker:
    ...
    df_contrib[col_renda] = pd.to_numeric(df_contrib[col_renda], errors="coerce").fillna(0)
    ...
    df_contrib = df_contrib[df_contrib["% da Meta Final"] > 0].sort_values("% da Meta Final", ascending=False)
    ...
    if not df_contrib.empty:
        ...
        fig_contrib = px.bar(
            ...
            text=df_contrib["% da Meta Final"].apply(lambda x: f"{x:.1f}%"),
        )
