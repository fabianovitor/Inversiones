...
# Linha a ignorar (índice baseado em 0, APÓS leitura do CSV, SEM contar o cabeçalho)
# Linha 18 da planilha = índice 16 no DataFrame (linha 1 = índice 0)
LINHA_IGNORAR = 16
...
def process_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Remove a linha problemática (índice 16 = linha 18 da planilha, sem contar cabeçalho)
    ...
    if col_ticker in df.columns:
        df = df[df[col_ticker].notna()]
        df = df[df[col_ticker].astype(str).str.strip() != ""]
        df = df[~df[col_ticker].astype(str).str.strip().isin(["nan", "NaN", "None"])]
...
with st.expander("🔍 Debug — Colunas encontradas na planilha"):
    st.write("**Colunas encontradas:**", list(df_raw.columns))
    st.write(f"**Total de linhas (raw):** {len(df_raw)}")
    st.write(f"**Linha ignorada (índice {LINHA_IGNORAR}):**")
    if LINHA_IGNORAR < len(df_raw):
        st.dataframe(df_raw.iloc[[LINHA_IGNORAR]])
...
for col in ["DY (%)", "% da Meta", "Yield on Cost"]:
    if col in df_display.columns:
