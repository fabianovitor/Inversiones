...
def get_ticker_info(ticker: str, rates: dict) -> dict:
    """
    Busca informações ao vivo via yfinance.
    Conversão de moeda aplicada APENAS aqui, sobre dados de mercado.

    Exemplos:
    - ERIC-B.ST → Yahoo Finance retorna preço em SEK → converte para USD
    - O, MAIN, VNQ → retornam em USD → sem conversão
    - PETR4.SA → retorna em BRL → converte para USD
    """
    try:
        t    = yf.Ticker(ticker)
        info = t.info
...
# Exibe taxa USD/BRL como referência informativa na sidebar
usd_brl_ref = rates.get("BRL", 0)
usd_sek_ref = rates.get("SEK", 0)

st.sidebar.success(f"✅ {len(df)} ativos carregados")
st.sidebar.caption(f"💱 USD/BRL: R$ {1/usd_brl_ref:.4f}" if usd_brl_ref > 0 else "")
st.sidebar.caption(f"💱 USD/SEK: kr {1/usd_sek_ref:.4f}" if usd_sek_ref > 0 else "")
