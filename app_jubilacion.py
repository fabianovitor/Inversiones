...
# Mapeamento: chave interna → nome real da coluna na planilha
COLUMN_MAP = {
    "ticker":      "Ticker",
    "nome":        "Empresa",
    "categoria":   "Categoría",
...
def process_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    """
    Processa o DataFrame da planilha.
    Retorna df com colunas calculadas adicionadas.
    As colunas originais da planilha são MANTIDAS com seus nomes originais.
    """
    df = df.copy()
    df.columns = [str(c).strip() for c in df.columns]
    df = df.dropna(how="all")

    col_ticker = COLUMN_MAP["ticker"]        # "Ticker"
    col_qtd    = COLUMN_MAP["quantidade"]    # "Cantidad"
    col_pm     = COLUMN_MAP["preco_medio"]   # "Precio Medio"
    col_div    = COLUMN_MAP["dividendos"]    # "Dividendos TTM"
    col_yoc    = COLUMN_MAP["yield_cost"]    # "Yield on Cost"
    col_pa     = COLUMN_MAP["preco_atual"]   # "Precio Actual"
    col_vt     = COLUMN_MAP["valor_total"]   # "Valor Total"
    ...
    # Converte colunas numéricas (cria cópias numéricas temporárias para cálculo)
    def get_num(col):
    ...
    if col_vt in df.columns:
        df["Patrimônio (USD)"] = vt_num.apply(
    ...
    # Prioridade: usar Dividendos TTM / 12
    # Alternativa: Patrimônio * YoC / 12
    rendas = []
    ...
    # DY (%)
    if col_yoc in df.columns:
        df["DY (%)"] = yoc_num

    # Filtra só ativos com patrimônio > 0
...
col_ticker = COLUMN_MAP["ticker"]    # "Ticker"
col_cat    = COLUMN_MAP["categoria"] # "Categoría"
...
with g3:
    ...
    if col_renda in df.columns and col_ticker in df.columns:
        ...
        if not df_renda.empty:
            ...
            st.info("Sem dados de renda disponíveis para exibir o gráfico.")
    else:
        st.info(f"Coluna '{col_renda}' não encontrada. Verifique se a planilha tem 'Dividendos TTM'.")
...
# df numérico para merge e ordenação
df_tabela = df.copy()

# Merge com % da Meta (ainda numérico)
...
for col in [col_pm, col_pa, col_vt, col_div]:
    if col in df_display.columns:
        df_display[col] = df_display[col].apply(
            lambda x: f"$ {float(x):,.2f}" if is_valid_number(x) else (str(x) if x is not None else "")
        )
