...
def load_sheet_raw(spreadsheet_id: str, gid: str = "0") -> str:
    """Carrega o CSV bruto da planilha."""
...
def find_bad_rows(csv_text: str) -> list:
    """
    Identifica linhas que têm formato diferente do cabeçalho.
    Retorna lista de índices (0-based, SEM contar o cabeçalho) para pular.
    """
    ...
    bad_rows = []
    for i, line in enumerate(lines[1:], start=0):  # i = índice no DataFrame (sem header)
        col_count = len(line.split(","))
        # Se a linha tem número muito diferente de colunas, é suspeita
        # Também verifica se parece ser uma linha de texto corrido (poucas colunas)
        if col_count < header_cols * 0.5:
            bad_rows.append(i)
...
@st.cache_data(ttl=300)
def load_sheet(spreadsheet_id: str, gid: str = "0") -> tuple:
    """Carrega a planilha, filtrando linhas problemáticas."""
    csv_text = load_sheet_raw(spreadsheet_id, gid)
    if not csv_text:
        return pd.DataFrame(), []
    
    # Detecta linhas problemáticas
    bad_rows = find_bad_rows(csv_text)
    
    try:
        # Tenta carregar normalmente primeiro
        df_full = pd.read_csv(StringIO(csv_text))
        
        # Remove linhas problemáticas detectadas
        if bad_rows:
            df_full = df_full.drop(index=bad_rows, errors='ignore').reset_index(drop=True)
        
        return df_full, bad_rows
    except Exception as e:
...
def is_valid_number(val):
    """Verifica se o valor é um número válido (não None e não NaN)."""
    if val is None:
        return False
    if isinstance(val, float) and pd.isna(val):
        return False
...
def is_valid_ticker(val):
    """Verifica se o valor parece ser um ticker válido."""
    if val is None:
        return False
    s = str(val).strip()
    if s in ("", "nan", "NaN", "None", "N/A", "#N/A"):
        return False
    # Ticker válido: alfanumérico, pontos, hífen, máximo ~10 chars
    if len(s) > 15:
        return False
...
def process_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Remove linhas sem ticker válido ANTES de converter colunas numéricas
    if col_ticker in df.columns:
        df = df[df[col_ticker].apply(is_valid_ticker)]
        df = df.reset_index(drop=True)

    # Converte colunas numéricas usando to_float seguro
    for col in [col_qtd, col_pm, col_div, col_yoc, col_pa, col_vt]:
        if col in df.columns:
            df[col] = df[col].apply(to_float)

    # Patrimônio = Valor Total da planilha (ou calcula se não existir)
    ...
    # Filtra apenas linhas com patrimônio válido e positivo
...
META_MENSAL_USD = st.sidebar.number_input(
    ...
    step=100.0,
    help="Sua meta final de renda passiva mensal em USD"
)

# Meta intermediária (opcional)
usar_meta_intermediaria = st.sidebar.checkbox(
    "Usar meta intermediária",
    value=False,
    help="Ative para definir uma meta de curto/médio prazo"
)
...
with st.spinner("📡 Sincronizando dados..."):
    rates  = get_all_rates()
    df_raw, linhas_ignoradas = load_sheet(SPREADSHEET_ID, GID_CARTERA)
...
with st.expander("🔍 Debug — Informações do carregamento"):
    st.write("**Colunas encontradas:**", list(df_raw.columns))
    st.write(f"**Total de linhas (após filtro automático):** {len(df_raw)}")
    if linhas_ignoradas:
        st.warning(f"⚠️ Linhas ignoradas automaticamente (índices): {linhas_ignoradas}")
        st.write("**Conteúdo das linhas ignoradas:**")
        # Recarrega raw para mostrar as linhas ignoradas
        csv_text = load_sheet_raw(SPREADSHEET_ID, GID_CARTERA)
        if csv_text:
            df_completo = pd.read_csv(StringIO(csv_text))
            for idx in linhas_ignoradas:
                if idx < len(df_completo):
                    st.dataframe(df_completo.iloc[[idx]])
    st.write("**Primeiras 5 linhas do DataFrame carregado:**")
...
with g3:
    if "Renda Mensal Est. (USD)" in df.columns and col_ticker in df.columns:
        ...
        if not df_renda.empty:
            ...
            # Linha da meta intermediária (se ativa)
            if META_INTERMEDIARIA:
                fig_renda.add_vline(
...
# Linha meta intermediária (se ativa)
if META_INTERMEDIARIA:
    fig_cenarios.add_hline(
...
if "Renda Mensal Est. (USD)" in df.columns and col_ticker in df.columns:
    ...
    if not df_contrib.empty:
        ...
        if META_INTERMEDIARIA:
            pct_inter_total = (renda_mensal / META_INTERMEDIARIA * 100) if META_INTERMEDIARIA > 0 else 0
            fig_contrib.add_hline(
                y=pct_inter_total / len(df_contrib) if len(df_contrib) > 0 else 0,
                line_dash="dot", line_color="#3b82f6",
                annotation_text=f"Ref. {nome_meta_intermediaria}"
            )
...
if df_contrib is not None and col_ticker in df_tabela.columns and "% da Meta Final" in df_contrib.columns:
    ...
    df_tabela = df_tabela.merge(
        df_contrib[cols_merge], on=col_ticker, how="left"
    )
