...
def process_dataframe(df: pd.DataFrame):
    ...
    def get_num_series(col):
        """Converte coluna para série numérica."""
    ...
    # ============================================================
    # RENDA MENSAL — lógica corrigida
    # ============================================================
    rendas = []
    
    if col_div and col_div in df.columns:
        # Usa Dividendos TTM / 12
        for i, div in enumerate(div_num):
            if is_valid_number(div):
                rendas.append(round(div / 12, 4))
            else:
                # Fallback: tenta calcular pelo YoC se disponível
                yoc_val = yoc_num.iloc[i] if col_yoc and col_yoc in df.columns else None
                pat_val = df["Patrimônio (USD)"].iloc[i]
                if is_valid_number(yoc_val) and is_valid_number(pat_val) and pat_val > 0:
                    rendas.append(round(pat_val * yoc_val / 100 / 12, 4))
                else:
                    rendas.append(0.0)
        df["Renda Mensal Est. (USD)"] = rendas
        
    elif col_yoc and col_yoc in df.columns:
        # Usa YoC sobre o patrimônio
        for pat, yoc in zip(df["Patrimônio (USD)"], yoc_num):
            if is_valid_number(pat) and is_valid_number(yoc) and pat > 0:
                rendas.append(round(pat * yoc / 100 / 12, 4))
            else:
                rendas.append(0.0)
        df["Renda Mensal Est. (USD)"] = rendas
    else:
    ...
    mask_pat = df["Patrimônio (USD)"].apply(lambda x: is_valid_number(x) and x > 0)
...
with st.expander("🔍 Debug — Resultado do processamento (clique para ver)", expanded=False):
    ...
    if "Renda Mensal Est. (USD)" in df.columns:
        ...
        if col_ticker:
            st.dataframe(df[[col_ticker, "Renda Mensal Est. (USD)", "Patrimônio (USD)"]].head(15))
    else:
