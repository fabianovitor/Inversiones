...
def render_progress_bar(label, valor_atual, meta, cor, nome_meta="Meta"):
    """Renderiza barra de progresso com texto legível."""
    progresso = min((valor_atual / meta) * 100, 100) if meta > 0 else 0.0
    falta = max(meta - valor_atual, 0)
    
    # Cor do texto: escuro para barras claras/pequenas, branco para barras grandes
    # Sempre usa texto escuro FORA da barra para legibilidade
    bar_width = progresso
...
# Debug compacto — oculto por padrão
...
# ─────────────────────────────────────────────────────────────────────────────
# PROGRESSO — BARRAS COM TEXTO LEGÍVEL
# ─────────────────────────────────────────────────────────────────────────────
...
render_progress_bar(
    label="Meta Final",
    valor_atual=renda_mensal,
    meta=META_MENSAL_USD,
    cor=cor_final,
    nome_meta="Meta Final"
)
...
if META_INTERMEDIARIA:
    ...
    render_progress_bar(
        label=nome_meta_intermediaria,
        valor_atual=renda_mensal,
        meta=META_INTERMEDIARIA,
        cor=cor_inter,
        nome_meta=nome_meta_intermediaria
    )
...
# ─────────────────────────────────────────────────────────────────────────────
# RENDA E PERFORMANCE — LABELS LEGÍVEIS
# ─────────────────────────────────────────────────────────────────────────────
    ...
    if "Renda Mensal Est. (USD)" in df.columns and col_ticker in df.columns:
        ...
        if not df_renda.empty:
            fig_renda = go.Figure(go.Bar(
                ...
                # Labels escuros para legibilidade
            ...
            fig_renda.update_layout(
                ...
                # Aumenta margem direita para labels não serem cortados
                margin=dict(r=80)
            )
...
with g4:
    if "Retorno (%)" in df.columns and col_ticker in df.columns:
        ...
        if not df_ret.empty:
            ...
            # Labels: sempre escuros, posicionados fora das barras
            text_vals = df_ret["Retorno (%)"].apply(
                lambda x: fmt_pct_sinal(x) if pd.notna(x) else ""
            )
            text_positions = [
                "outside" for _ in df_ret["Retorno (%)"]
            ]
            ...
            fig_ret.update_layout(
                ...
                # Margem extra para labels
                margin=dict(t=60, b=20)
            )
            # Ajusta range Y para dar espaço aos labels externos
...
if "Renda Mensal Est. (USD)" in df.columns and col_ticker in df.columns:
    ...
    if not df_contrib.empty:
        ...
        max_val = df_contrib["% da Meta Final"].max() if not df_contrib.empty else 0
        fig_contrib.update_yaxes(range=[0, max_val * 1.2])
