...
with g1:
    if col_ticker and "Patrimônio (USD)" in df.columns:
        ...
        if not df_plot.empty:
            ...
            st.plotly_chart(fig_pie, use_container_width=True, key="chart_pie_patrimonio")
        else:
    ...
    if col_cat and "Patrimônio (USD)" in df.columns:
        ...
        if not df_cat.empty:
            ...
            fig_setor.update_layout(**LAYOUT_BASE, title_font=dict(color="#1e293b", size=16), height=500,
                                    legend=dict(font=dict(color="#1e293b", size=12)))
            st.plotly_chart(fig_setor, use_container_width=True, key="chart_pie_categoria")
        else:
...
with g3:
    ...
    if col_renda in df.columns and col_ticker:
        ...
        if not df_renda.empty:
            ...
            st.plotly_chart(fig_renda, use_container_width=True, key="chart_bar_renda")
        else:
...
with g4:
    if "Retorno (%)" in df.columns and col_ticker:
        ...
        if not df_ret.empty:
            ...
            st.plotly_chart(fig_ret, use_container_width=True, key="chart_bar_retorno")
...
if col_ticker and col_ticker in df.columns:
    ...
    if ticker_selecionado:
        ...
        if not historico_preco.empty:
            ...
            st.plotly_chart(fig_hist, use_container_width=True, key=f"chart_hist_{ticker_selecionado}")
...
st.plotly_chart(fig_cenarios, use_container_width=True, key="chart_simulador_cenarios")
...
if col_renda in df.columns and col_ticker:
    ...
    if not df_contrib.empty:
        ...
        st.plotly_chart(fig_contrib, use_container_width=True, key="chart_contrib")
