...
# Mapeamento EXATO das colunas da planilha "Cartera"
COLUMN_MAP = {
    "ticker":      "Ticker",
    "nome":        "Nombre",
    "quantidade":  "Cantidad",
...
# Linha a ser ignorada (linha 16 da planilha = índice 14 no DataFrame)
LINHA_IGNORAR = 14
...
def get_exchange_rate(from_currency: str, to_currency: str = "USD") -> float:
    """
    Busca a taxa de câmbio entre duas moedas via yfinance.
    Exemplos: SEK→USD, EUR→USD, BRL→USD.
    Retorna 1.0 se não conseguir obter a taxa (fallback seguro).
    """
...
def get_all_rates() -> dict:
    """
    Busca as taxas de câmbio mais comuns para USD de forma agrupada.
    Evita múltiplas chamadas individuais à API.
    """
...
def convert_market_price_to_usd(valor: float, moeda: str, rates: dict) -> float:
    """
    Converte um preço obtido da INTERNET (yfinance) para USD.
    Usado EXCLUSIVAMENTE para dados de mercado ao vivo.
    Os dados da planilha NÃO passam por esta função.

    Exemplos de conversão:
    - Ericsson (ERIC-B.ST) retorna preço em SEK → converte para USD
    - Ativos americanos (USD) → retorna o valor sem alteração
    """
    moeda = str(moeda).upper().strip()
    if moeda == "USD" or moeda == "":
        return valor
    taxa = rates.get(moeda, None)
    if taxa is None:
        taxa = get_exchange_rate(moeda, "USD")
    return valor * taxa if taxa > 0 else valor


@st.cache_data(ttl=300)
def get_ticker_info(ticker: str, rates: dict) -> dict:
    """
    Busca informações atualizadas de um ticker via yfinance.
    Converte preço atual e DY para USD caso a moeda do ativo não seja USD.

    REGRA: conversão de moeda APENAS para dados vindos da internet.
    - Tickers americanos/REITs: código direto (ex: O, MAIN, VNQ)
    - Ericsson Estocolmo: ERIC-B.ST (retorna SEK → converte para USD)
    - Tickers brasileiros: código + .SA (ex: PETR4.SA)
    """
    try:
        t = yf.Ticker(ticker)
        info = t.info
        ...
        # Moeda que o Yahoo Finance usa para este ativo
        moeda_mercado = info.get("currency", "USD").upper()

        preco_raw = info.get("currentPrice") or info.get("regularMarketPrice", None)

        # Conversão de moeda APENAS para dados vindos da internet
        preco_usd = None
        if preco_raw:
            if moeda_mercado == "USD":
                preco_usd = float(preco_raw)          # já é USD, não converte
            else:
                taxa = rates.get(moeda_mercado, get_exchange_rate(moeda_mercado))
                preco_usd = float(preco_raw) * taxa   # converte SEK, BRL, EUR etc.

        # Variação do dia — percentual, não precisa de conversão de moeda
        variacao_dia = 0.0
        ...
        # DY — já é percentual, sem conversão necessária
        ...
        return {
            ...
            "preco_usd":     preco_usd,        # preço convertido para USD
            "preco_raw":     preco_raw,         # preço original na moeda do mercado
            "moeda_mercado": moeda_mercado,     # moeda original (SEK, USD, BRL, etc.)
            "dy_anual":      dy_anual,
...
def get_price_history(ticker: str, period: str = "1y") -> pd.DataFrame:
    """
    Busca o histórico de preços de um ticker via yfinance.
    O histórico é retornado na moeda original do ativo (sem conversão),
    pois serve apenas para visualização de tendência.
    """
    try:
        t = yf.Ticker(ticker)
...
def process_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    """
    Processa os dados da planilha 'Cartera'.

    REGRA ABSOLUTA: Todos os valores da planilha JÁ ESTÃO EM USD.
    Isso inclui ERIC.B, REITs, ETFs e qualquer outro ativo listado.
    NENHUMA conversão de moeda é aplicada aqui.
    Apenas limpeza numérica e cálculos derivados.
    """
    df = df.copy()
    df.columns = [str(c).strip() for c in df.columns]

    # ── Remove a linha 16 da planilha (índice 14) ──────────────────────────────
    ...
    # ── Limpeza de formatação de texto (remove $, %, vírgulas) ────────────────
    # Não altera os valores numéricos, apenas remove caracteres de formatação
    ...
    # ── Patrimônio = Cantidad × Precio Actual (já em USD) ─────────────────────
    if col_qtd in df.columns and col_pa in df.columns:
        df["Patrimônio (USD)"] = df[col_qtd] * df[col_pa]

    # ── Custo Total = Cantidad × Precio Promedio (já em USD) ──────────────────
    ...
    # ── Renda Mensal Estimada = Patrimônio × DY% / 12 ─────────────────────────
...
with st.spinner("📡 Sincronizando dados..."):
    rates   = get_all_rates()          # taxas de câmbio para conversão dos dados de internet
...
# Exibe taxa USD/BRL e USD/SEK como referência informativa na sidebar
...
# ═══════════════════════════════════════════════════════════════════════════════
# ANÁLISE DETALHADA POR ATIVO — DADOS AO VIVO COM CONVERSÃO AUTOMÁTICA
# ═══════════════════════════════════════════════════════════════════════════════
st.markdown("---")
...
if col_ticker in df.columns:
    ...
    if ticker_selecionado:
        ...
        # Exibe preço original E em USD se a moeda for diferente
        ...
        if not historico_preco.empty:
            ...
            # Label do eixo Y mostra a moeda original do ativo
...
# Inicializa df_contrib como None para verificação posterior
df_contrib = None
...
if "Renda Mensal Est. (USD)" in df.columns and col_ticker in df.columns:
    ...
    fig_contrib = px.bar(
        ...
        text=df_contrib["% da Meta"].apply(lambda x: f"{x:.1f}%"),
    )
...
with f2:
    # Monta lista de colunas disponíveis para ordenação
    colunas_ordenacao = [c for c in [
        "Patrimônio (USD)", "Renda Mensal Est. (USD)",
        "Retorno (%)", "DY (%)", "% da Meta"
    ] if c in df.columns or (df_contrib is not None and c in df_contrib.columns)]
    ordenar_por = st.selectbox("Ordenar por", options=colunas_ordenacao) if colunas_ordenacao else None

# Monta tabela com merge seguro
df_tabela = df.copy()

# Só faz o merge se df_contrib foi criado e tem a coluna necessária
...
# Colunas em USD para formatação
colunas_usd = [c for c in [
    "Patrimônio (USD)", "Custo Total (USD)",
    "Lucro/Prejuízo (USD)", "Renda Mensal Est. (USD)"
] if c in df_tabela.columns]

# Colunas percentuais para formatação
colunas_pct = {
    c: "{:+.2f}%" for c in ["Retorno (%)", "DY (%)", "% da Meta"]
    if c in df_tabela.columns
}

# Aplica formatação de forma segura
try:
    formato = {c: "$ {:,.2f}" for c in colunas_usd}
    formato.update(colunas_pct)

    styled = df_tabela.style.format(formato)

    if colunas_usd:
        styled = styled.background_gradient(subset=colunas_usd, cmap="Greens")

    st.dataframe(styled, use_container_width=True, height=420)
except Exception:
    # Fallback: exibe sem formatação especial se houver erro
    st.dataframe(df_tabela, use_container_width=True, height=420)
