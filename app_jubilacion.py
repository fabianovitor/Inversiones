...
def get_ticker(ticker):
    try:
        ...
        # yfinance às vezes retorna em % (ex: 4.5) e às vezes em decimal (0.045)
        if dy and dy > 1:
            dy = dy / 100

        div_rate = info.get("dividendRate")  # dividendo anual por ação na moeda original
        div_anual_usd = None
        if div_rate:
        ...
        return {"ticker": ticker, "nome": ticker, "preco_usd": None,
                "moeda": "USD", "var_pct": None, "div_yield": None, "div_anual_usd": None}
...
if debug:
    st.subheader("DEBUG - Colunas detectadas")
    st.write(list(df.columns))
    st.subheader("DEBUG - Primeiras linhas da planilha")
...
if debug:
    st.write(f"**Coluna ticker detectada:** `{col_ticker}`")
    st.write(f"**Coluna quantidade detectada:** `{col_qtd}`")
    st.write(f"**Coluna preço médio detectada:** `{col_pm}`")

if not col_ticker:
    st.error("Coluna de ticker nao encontrada na planilha. Ative o Debug para ver as colunas disponiveis.")
    st.stop()
...
if df_ok.empty:
    st.warning("Nenhum ativo encontrado na planilha.")
    st.stop()

if debug:
    st.subheader("DEBUG - Ativos filtrados")
    st.dataframe(df_ok)
...
if not df_principal.empty:
    ...
    df_show.columns = ["Ticker", "Nome", "Moeda", "Qtd", "PM (USD)", "Preco (USD)",
                       ...
                       "DY", "Renda Mes"]
    df_show["Qtd"] = df_show["Qtd"].apply(lambda x: f"{x:.0f}" if pd.notna(x) else "-")
...
if not df_eric.empty:
    ...
    df_e.columns = ["Ticker", "Nome", "Moeda", "Qtd", "PM (USD)", "Preco (USD)",
                    ...
                    "DY", "Renda Mes"]
    df_e["Qtd"] = df_e["Qtd"].apply(lambda x: f"{x:.0f}" if pd.notna(x) else "-")
