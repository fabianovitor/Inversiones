...
def process_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "Patrimônio (USD)" in df.columns and "Custo Total (USD)" in df.columns:
        ...
        mask = df["Custo Total (USD)"] > 0
        df["Retorno (%)"] = None
        df.loc[mask, "Retorno (%)"] = (
            (df.loc[mask, "Lucro/Prejuízo (USD)"] / df.loc[mask, "Custo Total (USD)"]) * 100
        ).round(2)
...
with g4:
    if "Retorno (%)" in df.columns and col_ticker in df.columns:
        ...
        df_ret = df_ret.sort_values("Retorno (%)", ascending=False)
        colors = [
...
if col_ticker in df.columns:
    ...
    if ticker_selecionado:
        ...
        pat_ativo = linha_ativo.get("Patrimônio (USD)", 0)
        renda_ativo = linha_ativo.get("Renda Mensal Est. (USD)", 0)
        pat_val = pat_ativo if pd.notna(pat_ativo) else 0
        renda_val = renda_ativo if pd.notna(renda_ativo) else 0
...
with f2:
    ...
    if df_contrib is not None:
        if "% da Meta" not in colunas_ordenacao and "% da Meta" in df_contrib.columns:
            colunas_ordenacao.append("% da Meta")
...
# ── Cria versão formatada para exibição (sem .style) ──
df_display = df_tabela.copy()

# Converte colunas numéricas para string formatada
