# ============================================================
# DASHBOARD DE APOSENTADORIA - ARQUIVO COMPLETO
# PARTE 1 de 2 - Cole esta parte primeiro
# ============================================================
...
def limpar_ticker(ticker_str):
    ...
    # Casos especiais com prefixo STO: (Bolsa de Estocolmo)
    if t.startswith("STO:"):
        ticker_limpo = t.replace("STO:", "")
        return ticker_limpo + ".ST"  # Adiciona sufixo do Yahoo
    return t
...
if debug:
    st.error("🔍 MODO DEBUG ATIVO - role para baixo para ver os botões de download")
...
with st.spinner("Buscando cotacoes no Yahoo Finance..."):
    for _, row in df_ok.iterrows():
        ...
        ticker = MAPEAMENTO_TICKERS.get(ticker_original, ticker_original)
        ...
        ativos.append({
            ...
            "ericsson": ticker.upper() == TICKER_ERICSSON.upper() or ticker_original in ["ERICB", "ERIC-B", "ERICSSON"],
        })

dfa = pd.DataFrame(ativos)

if debug:
    st.warning("⬇️ BOTÕES DE DOWNLOAD DEBUG ABAIXO ⬇️")
