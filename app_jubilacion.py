...
def find_column(df, candidates):
    """Encontra uma coluna no df dado uma lista de possíveis nomes."""
...
def process_dataframe(df: pd.DataFrame):
    """
    Processa o DataFrame. Retorna (df_processado, mapa_colunas).
    """
    df = df.copy()
    ...
    col_ticker = find_column(df, ["Ticker", "ticker", "TICKER", "Symbol", "symbol"])
    col_categoria = find_column(df, ["Categoría", "Categoria", "Category", "Tipo", "Type"])
    col_qtd = find_column(df, ["Cantidad", "Quantidade", "Quantity", "Qtd", "Qty"])
    col_pm = find_column(df, ["Precio Medio", "Preço Médio", "Average Price", "Avg Price", "Preco Medio"])
    col_div = find_column(df, ["Dividendos TTM", "Dividendos", "Dividends TTM", "Dividends", "Div TTM"])
    col_yoc = find_column(df, ["Yield on Cost", "YoC", "Yield Cost", "YOC"])
    col_pa = find_column(df, ["Precio Actual", "Preço Atual", "Current Price", "Price"])
    col_vt = find_column(df, ["Valor Total", "Total Value", "Value", "Total"])
    col_acao = find_column(df, ["Acción", "Ação", "Action"])
...
5. Volte aqui e clique em **"🔄 Atualizar Dados Agora"**
        """)
...
with st.expander("🔍 Debug — Colunas da planilha (clique para ver)", expanded=False):
    st.write(f"**Colunas encontradas ({len(df_raw.columns)}):**")
...
with st.expander("🔍 Debug — Dados processados (clique para ver)", expanded=False):
    st.write(f"**Mapeamento de colunas:** {mapa}")
    st.write(f"**Colunas calculadas:** {[c for c in df.columns if c not in df_raw.columns]}")
    st.write(f"**Total de ativos:** {len(df)}")
    st.write(f"**Coluna Renda Mensal presente:** {'Renda Mensal Est. (USD)' in df.columns}")
    if "Renda Mensal Est. (USD)" in df.columns:
        st.write(f"**Valores de Renda Mensal:**")
        if col_ticker:
            st.dataframe(df[[col_ticker, "Renda Mensal Est. (USD)"]].head(10))
        else:
            st.dataframe(df[["Renda Mensal Est. (USD)"]].head(10))
...
with g1:
    if col_ticker and "Patrimônio (USD)" in df.columns:
        ...
        st.info(f"Coluna ticker='{col_ticker}' ou Patrimônio não encontrada.")
...
with g2:
    if col_cat and "Patrimônio (USD)" in df.columns:
        ...
        if not df_cat.empty:
            ...
            st.info(f"Sem dados de categoria (col='{col_cat}') para o gráfico.")
    else:
        st.info(f"Coluna de categoria não encontrada. col_cat='{col_cat}'")
    ...
    if col_renda in df.columns and col_ticker:
        ...
        if not df_renda.empty:
            ...
            fig_renda = go.Figure(go.Bar(
                x=df_renda[col_renda],
                y=df_renda[col_ticker],
                orientation="h",
                marker_color="#22c55e",
                text=df_renda[col_renda].apply(
            ...
            fig_renda.update_layout(
                **layout_eixos("USD / mês", ""),
                ...
                height=500,
                margin=dict(t=70, b=60, l=100, r=120),
            )
            st.plotly_chart(fig_renda, use_container_width=True)
        else:
            st.warning(f"Coluna '{col_renda}' existe mas não tem dados válidos.")
            st.write("Amostra dos dados de renda:", df[[col_ticker, col_renda]].head(5))
    else:
        st.warning(f"Coluna '{col_renda}' não foi calculada. Verificar colunas de dividendos/YoC.")
        st.write(f"col_div={col_div}, col_yoc={col_yoc}")
...
if col_ticker and col_ticker in df.columns:
    ...
    if ticker_selecionado:
        ...
        delta_preco = (
            ...
            if moeda_mercado != "USD" and preco_raw else None
        )
...
if col_renda in df.columns and col_ticker:
    ...
    if META_MENSAL_USD > 0:
        df_contrib["% da Meta Final"] = (
            df_contrib[col_renda] / META_MENSAL_USD * 100
        ).round(2)
    else:
    ...
    if META_INTERMEDIARIA and META_INTERMEDIARIA > 0:
        df_contrib["% da Meta Inter."] = (
            df_contrib[col_renda] / META_INTERMEDIARIA * 100
        ).round(2)
        ...
        fig_contrib.update_layout(
            **layout_eixos("", "% da Meta Final"),
            ...
            yaxis=dict(
                ...
                tickfont=dict(color="#1e293b", size=12),
                gridcolor="#e2e8f0",
                range=[0, max(max_val * 1.3, 5)],
            ),
            coloraxis_showscale=False,
        )
        st.plotly_chart(fig_contrib, use_container_width=True)
    else:
        st.info("Sem dados válidos para o gráfico de contribuição.")
else:
    st.info(f"Renda Mensal não calculada. col_ticker='{col_ticker}', col_renda presente={col_renda in df.columns}")
...
# Formata colunas originais da planilha
for col in [col_pm, col_pa, col_vt, col_div]:
    if col and col in df_display.columns:
        df_display[col] = df_display[col].apply(
            lambda x: f"$ {float(x):,.2f}" if is_valid_number(to_float(x)) else (str(x) if x is not None else "")
        )
if col_yoc and col_yoc in df_display.columns:
    df_display[col_yoc] = df_display[col_yoc].apply(fmt_pct)
